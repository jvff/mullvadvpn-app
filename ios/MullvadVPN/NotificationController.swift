...
class NotificationController: UIViewController, NotificationManagerDelegate {
    ...
    private func waitForVoiceOverToFinish(_ block: @escaping () -> Void) {
        removeVoiceOverObserver()
        voiceOverObserver = NotificationCenter.default.addObserver(forName: UIAccessibility.announcementDidFinishNotification, object: nil, queue: .main) { note in
            block()
        }
    }

    private func removeVoiceOverObserver() {
        if let voiceOverObserver = voiceOverObserver {
